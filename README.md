...
A production-grade, privacy-first NLP engine that runs entirely on-device — no server, no network, no data leaving the device. Built with DistilBERT → Core ML, SwiftUI, Apple Natural Language framework, and a hardware-aware adaptive inference pipeline.
...
```python
MultiTaskNLPModel
├── DistilBertModel (shared encoder, 6 layers, 768 hidden)
├── shared_projection: Linear(768→512) + GELU + Dropout
└── task_heads (ModuleDict):
    ├── sentiment: Linear(512→256) + GELU + Dropout + Linear(256→3)
    ├── intent:    Linear(512→256) + GELU + Dropout + Linear(256→5)
    ├── emotion:   Linear(512→256) + GELU + Dropout + Linear(256→6)
    └── toxicity:  Linear(512→256) + GELU + Dropout + Linear(256→3)
```
...
```
Privacy-Preserving Multi-Task NLP System for iOS | Swift, SwiftUI, CoreML, Apple Natural Language, PyTorch

• Designed and deployed a multi-task on-device NLP engine supporting sentiment analysis, intent detection,
  emotion classification, and toxicity detection with real-time inference and no server dependency.

• Converted and optimized a pretrained DistilBERT transformer from PyTorch to Core ML, applying compression
  techniques to reduce model footprint from 255 MB to 94 MB while preserving 98.2% accuracy.

• Engineered hybrid inference pipeline combining Apple Natural Language embeddings with Core ML transformer
  execution for adaptive routing and sub-40 ms latency on Apple Neural Engine.

• Built hardware-aware adaptive execution selecting model compression variant (FP16/INT8/6-bit) by device
  tier, with async SwiftUI pipeline and graceful Apple NL fallback paths.
```
